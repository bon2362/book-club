import { cleanupTrackedAuditRows } from '../../fixtures'
...
test('matching events и audit фиксируют бизнес-изменения, но heartbeat не создаёт шум', async ({
  ...
  const rename = await participantA.request.post(`/api/matching/sessions/${session.id}/join`, {
  ...
  await choose(participantA.request, session.id, firstBookCircle!.circleKey)
  current = await state(participantA.request, session.id)
  const cancel = await participantA.request.delete(`/api/matching/sessions/${session.id}/confirmation`, {
    data: { expectedStateVersion: current.session.stateVersion },
  })
  expect(cancel.ok()).toBe(true)
  await choose(participantA.request, session.id, firstBookCircle!.circleKey)
  await choose(participantA.request, session.id, secondBookCircle!.circleKey)
  await choose(participantA.request, session.id, firstBookCircle!.circleKey)

  const removeBook = await participantB.request.delete(`/api/matching/books/${books[0].id}`)
  ...
  await choose(participantC.request, session.id, confirmationKey!)
  const lockedResponse = await admin.request.get(`/api/admin/matching/sessions/${session.id}/locked-circles`)
  ...
  const dissolve = await admin.request.post(
  ...
  const freeze = await admin.request.post(`/api/matching/sessions/${session.id}/freeze`)
  expect(freeze.ok(), await freeze.text()).toBe(true)

  const analyticsResponse = await admin.request.get(
  ...
  const userAuditResponse = await admin.request.get(
  ...
  const matchingAuditResponse = await admin.request.get(
    `/api/admin/audit-log?actorUserId=${admin.userId}&pageSize=200`,
  )
  expect(matchingAuditResponse.ok(), await matchingAuditResponse.text()).toBe(true)
  ...
  async function actorAudit(actorUserId: string): Promise<AuditEvent[]> {
    const response = await admin.request.get(`/api/admin/audit-log?actorUserId=${actorUserId}&pageSize=200`)
