...
// Если в текущих данных нет книг с длинным описанием — тесты пропускаются.
...
test.describe('карточка книги — разворачивание описания', () => {
  ...
  test('кнопка «Читать далее» присутствует хотя бы на одной карточке', async ({ page }) => {
    const count = await page.getByRole('button', { name: /читать далее/i }).count()
    test.skip(count === 0, 'Нет книг с описанием длиннее 120 символов в текущих данных')
