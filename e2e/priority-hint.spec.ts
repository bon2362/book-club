...
async function setupProfileAndClearHint(page: import('@playwright/test').Page) {
  ...
  await saveProfile(page, testName, testContact)
...
test('тост появляется после второй книги', async ({ page }) => {
  ...
  // Ограничиваем локатор карточками каталога: после добавления книги на странице
  // появляется ещё одна кнопка «Хочу читать» — селект статуса чтения из #561
  // (хочу читать → читаю сейчас → прочитал:а). Она идёт раньше в DOM, поэтому
  // .first() без этого ограничения кликал по статусу, а не по следующей книге.
  const bookButtons = page.locator('article').getByRole('button', { name: /хочу читать/i })
