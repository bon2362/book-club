...
test('тост закрывается автоматически через 10 секунд', async ({ page }) => {
  await setupProfileAndClearHint(page)
...
test('hover на тост ставит таймер на паузу', async ({ page }) => {
  ...
  // Удерживаем курсор дольше штатных 10 секунд — тост обязан остаться
  await toast.hover()
  ...
  await expect(toast).toBeHidden({ timeout: 11000 })
})
