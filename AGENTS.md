...
Согласование времени встреч: `/calendar/<slug>`, подробности — `docs/features/calendar.md`.

- Свободное время **глобальное у пользователя**, страница — **на круг**. Адрес выводится из пары «книга + номер круга», а не из id строки `matching_circles`: круги пересобираются при каждой новой записи на книгу.
- Знаменатель везде — **отметившиеся**, а не размер круга. Тот, кто ни разу не открывал календарь, не блокирует назначение встречи. Это правило продублировано в `overlap.ts`, в тоне клетки и в попапе — расхождение между ними уже приводило к багу.
- Всё, что видит пользователь, рисуется **в его часовом поясе** (`lib/calendar/timezone.ts`); в базе — абсолютные `timestamptz`. Жёсткий `timeZone: 'UTC'` в компонентах календаря запрещён, это был баг.
- Тон клетки продублирован в `data-tone` / `data-free` / `data-marked`. **Проверять оттенок через `toHaveStyle` нельзя**: jsdom не разбирает `color-mix()` и выбрасывает значение, поэтому такая проверка проходит при любом цвете и не проверяет ничего.
- Миграция `0062` на прод не применяется автоматически; до её прогона чтение отдаёт заглушку, мутации — `409`.
...
- `lib/calendar/` — календарь круга: чистая логика интервалов, наложения, часовых поясов и адресов
- `app/calendar/[slug]/page.tsx` — страница согласования времени встречи круга
- `components/nd/CalendarClient.tsx` — состояние страницы календаря, автосохранение
...
Весь сайт использует **один набор семантических токенов и примитивов**. Контентные экраны следуют белому редакторскому канону; `/matching` имеет санкционированный opt-in «мягкий дашборд», описанный в `docs/features/matching.md` и также использующий только токены из `app/globals.css`.
**Источники правды:** значения токенов — `app/globals.css` (`:root`); живая витрина общих примитивов — страница `/styleguide`; исключения поверхности Matching — `docs/features/matching.md`. Перед версткой смотри оба релевантных источника, не изобретай примитивы заново.
...
2. **Геометрия через токены.** Контентные экраны используют `var(--radius)`; Matching может использовать `--radius-card`/`--radius-control`. Никаких литеральных `rounded-*`. Круг разрешён только для аватаров участников.
3. **Тени через токены.** Контентные экраны остаются плоскими; Matching может использовать `--shadow-card`/`--shadow-cover`. Не добавлять `box-shadow` с литералами.
...
### Токены
**Полный список токенов и их значения — в `app/globals.css` (`:root`). Не дублировать здесь** — это единственный источник значений, бери актуальные значения оттуда. Семантика: `--bg*` (фоны), `--text*` (текст по контрасту), `--accent` / `--success` (статусы), `--border*` (линии), `--nd-serif` / `--nd-sans` (шрифты), `--radius*` и `--shadow*` (геометрия санкционированных поверхностей).
...
### Примитивы
**Не изобретать — копировать с `/styleguide`** (живая витрина общих кнопок, инпутов, чипов, меток тира, карточек, аватаров). Для Matching дополнительно переиспользовать примитивы и правила из `docs/features/matching.md`; не переносить локальные цвета, радиусы или тени из прототипов.
