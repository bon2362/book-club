...
- **`NEXTAUTH_TEST_MODE=true`** — включает test-only auth/session endpoints и E2E fixture hooks
...
- **Именование файлов:**
  ...
  - Утилиты и хелперы: `kebab-case.ts` (например `books-with-covers.ts`)
...
- **Firewall в devcontainer** — разрешены только GitHub, npmjs.org, api.anthropic.com,
  vercel.com; exit code 7 = заблокировано firewall
...
- Обложки хранятся в `books.cover_url`
- `lib/covers.ts` **удалён** — не восстанавливать
- Таблица `book_covers` удалена из актуальной схемы — не восстанавливать
...
- `book_covers` была устаревшим кешем внешних обложек и удалена из актуальной схемы
...
- `NEXTAUTH_TEST_MODE` влияет на auth/session test hooks — помнить об этом в тестах
