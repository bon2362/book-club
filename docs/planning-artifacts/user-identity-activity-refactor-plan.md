...
### Follow-up D. Удаление legacy auth cache из `users`

Дата решения: 2026-05-23

Контекст: после релизов A-C и отдельной миграции `users.email -> contact_email + user_identities` таблица `users` должна оставаться профилем участника, а auth/provider-specific данные должны жить в `user_identities`.

Решение:

- `user_identities.telegram_username` оставить источником правды для Telegram auth/identity;
- `users.telegram_username` удалить из схемы и продовой БД;
- `users.auth_provider` удалить из схемы и продовой БД;
- `users.last_sign_in_at` удалить из схемы и продовой БД;
- provider и last sign-in display читать из `user_identities.provider` и `user_identities.last_seen_at`;
- `users.contacts` считать preferred contact пользователя: это ручное поле, которое пользователь может редактировать сам;
- при первом Telegram-входе автоматически заполнить `users.contacts = @telegramUsername`, если Telegram вернул username и `contacts` пустой;
- при последующих Telegram-входах не перетирать `users.contacts`, если пользователь уже указал контакт;
- в профиле пользователя и админке поле Telegram выводить из `users.contacts`;
- технический auth username из `user_identities.telegram_username` на сайте не показывать.

Почему:

- `users.telegram_username` дублирует provider-specific поле из `user_identities`;
- `users.auth_provider` дублирует `user_identities.provider` и хранит исторически смешанные значения (`telegram` и `telegram-preauth`);
- `users.last_sign_in_at` дублирует `user_identities.last_seen_at`;
- дублирование может разъехаться, если обновится только одна таблица;
- наличие Telegram username в `users` снова смешивает профиль пользователя и способ входа;
- при нескольких identities у одного пользователя `users.telegram_username` не сможет честно выразить, к какой identity относится значение.

Граница ответственности после рефакторинга:

- `users.name` — отображаемое имя участника;
- `users.contacts` — preferred Telegram/contact пользователя, редактируемый пользователем и показываемый в UI/админке;
- `user_identities.telegram_username` — Telegram username, полученный из Telegram auth, только для identity/auth/audit;
- `user_identities.provider_account_id` — canonical Telegram account id для auth lookup;
- `user_identities.last_seen_at` — последняя фиксация этой Telegram identity.
- `user_identities.provider` — источник правды для способа входа;
- `user_identities.last_seen_at` — источник правды для последнего входа через конкретную identity.

План реализации:

1. Найти все обращения к `users.telegramUsername` / `telegram_username`, `users.authProvider` / `auth_provider`, `users.lastSignInAt` / `last_sign_in_at`.
2. Обновить admin/user queries так, чтобы поле Telegram в UI строилось из `users.contacts`:
   - `contacts` нормализовать через `formatTelegramDisplay`/`normalizeTelegramContact`;
   - не использовать `user_identities.telegram_username` для публичного/display Telegram;
   - не считать `contacts` auth-identity.
3. Обновить `/api/me` и профильные ответы:
   - отдавать Telegram/contact display из `users.contacts`;
   - не отдавать технический auth username из `user_identities.telegram_username` в UI API, если он не нужен для клиентской логики;
   - legacy fallback на `users.telegram_username` держать только до миграции, если нужен безопасный промежуточный релиз.
4. Обновить auth callbacks:
   - при Telegram sign-in всегда писать username в `user_identities`;
   - при создании Telegram-пользователя или при пустом `users.contacts` заполнить `contacts = @telegramUsername`;
   - не обновлять `users.telegram_username`;
   - не обновлять `users.auth_provider`;
   - не обновлять `users.last_sign_in_at`;
   - не перетирать непустой `users.contacts`.
5. Обновить test helpers и E2E cleanup:
   - тестовые Telegram users должны удаляться через `user_identities.provider/provider_account_id` или `user_identities.telegram_username`;
   - не полагаться на `users.telegram_username`.
6. Добавить миграцию:
   - перед drop при необходимости backfill `user_identities.telegram_username` из `users.telegram_username` для Telegram identities, где поле пустое;
   - убедиться, что для пользователей с `users.auth_provider`/`users.last_sign_in_at` есть соответствующая `user_identities` запись;
...
7. Удалить поле из Drizzle schema и типов.
8. Обновить unit/e2e тесты админки, профиля и Telegram auth.
...
- `npm run typecheck`;
- `npm test`;
- `npm run test:e2e -- e2e/telegram-auth.spec.ts e2e/admin-users.spec.ts e2e/profile.spec.ts`;
- полный `npm run test:e2e`, если затронуты общие test helpers или admin cleanup.

Инварианты после релиза:

- колонки `user.telegram_username` в БД нет;
- колонки `user.auth_provider` в БД нет;
- колонки `user.last_sign_in_at` в БД нет;
- все Telegram identities имеют корректный `provider_account_id`;
- все пользователи имеют хотя бы одну `user_identities` запись;
- Telegram display в админке и профиле берётся из `users.contacts`;
- последний способ входа и last sign-in в UI/API берутся из `user_identities`;
- ручной `contacts` не перетирается Telegram auth;
- Telegram login после изменения username обновляет `user_identities.telegram_username`.
