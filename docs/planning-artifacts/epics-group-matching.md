...
FR26: Админ может открыть вид любого участника через `?as=<userId>` — read-only: личный список, ранги, сценарии, «Мои ходы» этого участника.
FR27: Админ в режиме `?as=` не может мутировать данные участника (сервер блокирует все POST/PATCH/DELETE при наличии `as`).
...
NFR7: (Security) Все мутации `/api/matching/*` отвергаются при наличии `?as=` в запросе (проверка в shared middleware, не в каждом хендлере).
...
UX-DR14: При активном `?as=<userId>` страница помечена явным баннером «Просмотр от лица X (read-only)».
...
FR26 → Epic 4 (?as= view)
FR27 → Epic 4 (mutation block при ?as=)
...
### Epic 4: Admin observability — impersonated read-only view

Админ открывает страницу через `?as=<userId>` и видит то, что видит участник: его личный список, ранги, сценарии, «Мои ходы». Shared middleware блокирует все мутации в режиме `?as=` и обрабатывает silent downgrade для не-админов. Каждое успешное чтение в `?as=` логируется в `admin_views`. UI помечен баннером «Просмотр от лица X (read-only)». Доступна admin-панель audit-логов для текущей сессии.
...
Админ открывает `/matching?as=<userId>` и видит то, что видит участник: личный список, ранги, сценарии, `Мои ходы`. Shared middleware блокирует все мутации в режиме `?as=` и обрабатывает silent downgrade для не-админов. Каждое успешное чтение логируется в `admin_views`. UI помечен баннером «Просмотр от лица X (read-only)». Доступна admin-панель audit-логов для текущей сессии.

### Story 4.1: Shared middleware — role check, `?as=` handling, mutation block

As a разработчик,
I want единый middleware для всех `/api/matching/*` маршрутов, обрабатывающий роль, `?as=` и блокировку мутаций,
...
**And** если `mutates: true` — middleware немедленно возвращает 403 «mutations disabled in admin view»
...
**When** запускается `npm test`
**Then** покрыты: не-аутентифицированный → 401; админ с `?as=` на мутирующем → 403; не-админ с `?as=` → silent downgrade; нет активной сессии на мутирующем → 409; frozen на мутирующем → 409
...
**When** страница рендерится
**Then** виден явный баннер вверху «Просмотр от лица *Мария · Барсук* (read-only)» с действием «Выйти из режима просмотра»
**And** drag-handles, кнопки «Удалить», «Хочу читать», поля действий — скрыты
