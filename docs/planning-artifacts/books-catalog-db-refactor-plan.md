...
Статус: refactor выполнен и задеплоен. Каталог книг переведён на таблицу `books`; legacy таблицы/поля `book_statuses`, `book_new_flags`, `book_covers`, `signup_books.book_name`, `book_priorities.book_name`, `session`, `account` удалены из production.
...
- удалить `books.canonical_key`;
- удалить `books.legacy_sheets_row_id`;
- удалить `books.source_submission_id`;
