...
Статус: обновлено после merge PR #141 (`9b3251d`) в `main`.
...
## Текущее состояние после PR #141

Выполнено и смержено в `main`:

- Этап 0: аудит данных и snapshot каталога.
...
- Этап 1: базовая схема и backfill.
...
- Этап 5: approval flow заявок переведён на `books`.
  ...
  - Автор заявки записывается на книгу с dual-write `book_name` + `book_id`.
  - Rename approved submission больше не делает blanket update по title, а ограничен конкретным `book_id`.
- Этап 7, phase 1: legacy `book_statuses` и `book_new_flags` удалены из runtime/schema/production.
  - Миграция `0022_drop_book_statuses_flags.sql` применена к production.
  - `/api/admin/book-status` и `/api/admin/book-new-flag` пишут напрямую в `books.reading_status` / `books.is_new`.
- Добавлена будущая guard migration `0023_books_catalog_drop_book_name.sql`.
  - Она не применена.
  - Она должна падать, если остаются строки без `book_id`, а не удалять пользовательские данные.
...
- Этап 3: полностью перевести пользовательский runtime на `book_id`.
  - Client должен передавать `bookId`, а не название.
  - `signup_books` и `book_priorities` должны записываться и читаться по `book_id`.
  - Legacy `book_name` можно оставить только как временный cache до применения `0023`.
- Этап 4: новая админская вкладка `Книги`.
  - CRUD/edit/publish/hide/soft delete.
  - Управление `visibility`, `reading_status`, `is_new`, `sort_order`.
- Этап 6: полностью удалить Google Sheets из runtime каталога.
  - Сейчас каталог уже из БД, но код/доки/env вокруг Sheets ещё нужно дочистить.
- Этап 7, phase 2: удалить оставшиеся legacy поля/routes после Этапа 3.
  - `signup_books.book_name`;
  - `book_priorities.book_name`;
  - `/api/admin/rename-book` и title-based helper paths;
  - применить `0023_books_catalog_drop_book_name.sql` только после production verification.
...
- `codecov/patch` у PR #141 остался красным из-за diff coverage, но `ci` и Vercel были зелёными перед merge.
- Следующий PR должен начинаться с production verification:
...
## Этап 3. Переключить пользовательские связи на `book_id` — TODO
...
## Этап 4. Админская вкладка “Книги” — TODO
...
## Этап 6. Удалить runtime-зависимость от Google Sheets — PARTIAL
...
### PR 3. Switch signup/priorities runtime to `book_id` — NEXT

- обновить signup/priorities APIs и UI;
- запретить новые записи без `book_id`;
- оставить legacy поля временно.

Обязательные детали для следующего PR:

- API должен принимать `bookId`, а legacy `bookName` оставить только для backward compatibility на переходный период;
- UI selectedBooks/priorities должен хранить id, а отображаемое название брать из `books`;
- `app/api/signup`, `app/api/priorities`, admin drawer/actions должны перейти на `book_id`;
- e2e обязательно: выбор книги -> reload -> запись сохранилась; приоритеты -> reload -> порядок сохранился;
- перед применением `0023` проверить production counts по `book_id is null`.

### PR 4. Admin books UI — TODO
...
### PR 6. Remove Sheets and legacy tables — PARTIAL / TODO

- удалить Google Sheets runtime code для книг;
- удалить `book_statuses`, `book_new_flags` — DONE in PR #141 / migration `0022`;
- подтвердить отсутствие/удалить `book_covers`;
- удалить legacy `book_name` поля — TODO after PR 3 / migration `0023`;
