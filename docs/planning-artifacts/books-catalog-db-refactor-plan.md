...
Дата: 2026-05-23
Актуализировано: 2026-05-25

Статус: основной refactor выполнен и задеплоен. Каталог книг переведён на таблицу `books`; legacy таблицы/поля `book_statuses`, `book_new_flags`, `book_covers`, `signup_books.book_name`, `book_priorities.book_name`, `session`, `account` удалены из production. Финальный cleanup 2026-05-25 убирает оставшиеся временные поля каталога: `books.size`, `books.archived_at`, `source = 'sheets_import'`, owner-specific PostHog opt-out и добавляет сохранение активной вкладки админки в URL.
...
- Поля книги должны редактироваться полностью: название, автор, тема/теги, тип, страницы, дата публикации, ссылка на текст, описание, обложка, объяснение “зачем читать”, рекомендационная ссылка, статусы и публикация.
...
- `hidden` — единственное состояние скрытия книги от публичного каталога;
- архивного состояния больше нет: `archived_at` удаляется как дублирование `visibility = 'hidden'`;
...
### `legacy_book_mappings` — удалена после миграции

Временная миграционная таблица использовалась только для безопасного переноса старых ссылок.
...
- после успешного backfill и удаления legacy `book_name` таблица больше не нужна и удаляется финальным cleanup.
...
## Итоговое состояние после PR #148
...
- Этап 6: Google Sheets удалён из runtime каталога — DONE in PR #144.
  ...
  - `GOOGLE_SHEETS_ID` / `GOOGLE_SERVICE_ACCOUNT_KEY` удалены из env schema после финального cleanup.
  - `lib/sheets.ts` удалён после финального cleanup.
...
- Этап 7, phase 2: legacy physical cleanup — DONE in PR #147.
  - Runtime больше не пишет `book_name`.
  - Drizzle schema больше не содержит `book_name` в `signup_books` / `book_priorities`.
  - Миграция `0024_drop_legacy_book_name.sql` применена к production.
  - Физические production-колонки `book_name` удалены.
  - `book_covers` удалена из production.
...
Финальный cleanup выполнен:

- Этап 7, phase 2: удалить физические legacy поля после production soak — DONE in PR #147 / migration applied to production.
...
- Этап 7, phase 3: финально дочистить legacy mentions — DONE in PR #147.
  - `scripts/cleanup-test-books.ts` переведён на `book_id`.
  - `scripts/generate-books-migration.ts` и `scripts/books-catalog-audit.ts` удалены после финального cleanup.
  - docs/testing/AGENTS/CLAUDE обновлены под DB fixtures.
- Этапы 8-9: legacy `session` и `account` удалены в PR #148; migration `0025_drop_legacy_auth_tables.sql` применена к production.
- Этап 10: финальная чистка docs/tests/feature docs — DONE.
- Этап 11: migration-only fields удалены.
  - `books.canonical_key`;
  - `books.legacy_sheets_row_id`;
  - `books.source_submission_id`;
...
- Финальная production verification после PR #148:
  - `signup_books.book_name` и `book_priorities.book_name` физически отсутствуют;
  - `book_statuses`, `book_new_flags`, `book_covers`, `session`, `account` физически отсутствуют;
  - `books.canonical_key`, `books.legacy_sheets_row_id`, `books.source_submission_id` и `legacy_book_mappings` физически отсутствуют после Этапа 11;
  - `__test_book_*` в production-каталоге отсутствуют;
  - `user_identities` содержит активные auth identities; `verificationToken` оставлена для magic link.
...
- книги из Sheets на финальном cleanup считаются админскими и получают `source = 'admin'`;
...
- временный `canonical_key` использовался только для аудита и затем был удалён;
...
- `book_name` больше не пишется runtime-кодом, не описан в Drizzle schema и физически удалён из production DB в Этапе 7.
...
- список книг:
  ...
  - бейдж источника: `admin`, `submission`;
...
- удаление/архив:
  - отдельного archive/restore больше нет;
  - `hidden` означает “не показывать в каталоге”, но книга остаётся активной административной сущностью;
  - физическое удаление не делать в обычном UI, потому что даже hidden книга может иметь историю signup/priorities.
...
- Реализованы CRUD/edit/publish/hide; archive/restore удаляется финальным cleanup как лишнее состояние.
...
- удалить `lib/sheets.ts`, если он больше нигде не нужен;
- удалить `GOOGLE_SHEETS_ID` и `GOOGLE_SERVICE_ACCOUNT_KEY` из env schema, если они не используются другими фичами;
...
- Google Sheets env vars удалены из env schema.
- `lib/sheets.ts` удалён.

## Этап 7. Удалить legacy таблицы и поля — DONE
...
- legacy API:
  - `/api/admin/book-status` — DONE, deleted in PR #147;
  - `/api/admin/book-new-flag` — DONE, deleted in PR #147;
  - `/api/admin/rename-book` — DONE, deleted in PR #146;
- код каскадного переименования в submission/admin routes — DONE;
- любые helper-и, которые ищут книгу по названию как primary mechanism — DONE for runtime; historical migration scripts помечены deprecated.
...
## Этап 8. Удалить legacy `session` table — DONE in PR #148
...
- production audit: `select count(*) from "session"` вернул `0`;
- `IdentityAwareDrizzleAdapter` больше не передаёт `sessionsTable` в `DrizzleAdapter`;
- `sessions` удалена из `lib/db/schema.ts`;
- test endpoints больше не читают `session`;
- миграция `0025_drop_legacy_auth_tables.sql` применена к production и удалила таблицу после guard-проверки.

Выполнено:

- подтверждено в production, что `session` пуста:
  - `select count(*) from "session";`
- заменён adapter path так, чтобы JWT strategy не требовала DB session table;
- удалены runtime imports `sessions`;
- `/api/test/user` сохраняет совместимый response shape (`sessionCount: 0`) без чтения таблицы;
- добавлена и применена миграция `DROP TABLE IF EXISTS "session"`;
- обновлены feature docs.
...
## Этап 9. Удалить legacy `account` table — DONE in PR #148

Цель: сделать `user_identities` единственным источником внешних auth identity и убрать техническую таблицу Auth.js `account`.

Проверенное состояние:

- production audit: 4 Google rows в `account`, все имеют matching `user_identities(provider='google')`;
- `account` больше не является fallback lookup в `IdentityAwareDrizzleAdapter`;
- `linkAccount` создаёт/обновляет `user_identities`, а не `account`;
- `resolveOrCreateUserFromIdentity` больше не пишет в `account`;
- Telegram и email identities не зависят от `account`;
- миграция `0025_drop_legacy_auth_tables.sql` применена к production и удалила таблицу только после проверки matching identity.
...
- проведён audit:
  ...
  - production `accountsWithoutIdentity = 0`;
- доработан `IdentityAwareDrizzleAdapter`:
  ...
  - verification token flow для magic link реализован напрямую через `verificationToken`;
- Google One Tap / OAuth helper больше не вставляет `accounts`;
- `accounts` удалена из `lib/db/schema.ts`;
- добавлена и применена миграция `DROP TABLE IF EXISTS "account"`;
- обновлены tests/docs.
...
## Этап 10. Финальная чистка тестов и документации — DONE

Задачи:

- обновить unit tests — DONE:
  - books repository;
...
- удалить legacy поля после production soak.
...
- Legacy `book_name` физически удалён из production DB в PR #147.

### PR 4. Admin books UI — DONE in PR #143

- новая вкладка `Книги`;
- CRUD/edit/publish/hide;
- archive/restore через `archived_at` был добавлен в PR #143, но признан лишним и удаляется финальным cleanup;
...
### PR 6. Remove Sheets and legacy tables — DONE in PR #144, PR #146, PR #147

- удалить Google Sheets runtime code для книг — DONE in PR #144;
- удалить leftover Google Sheets helper/dependency/env — DONE in PR #9 cleanup;
...
- удалить legacy routes:
  ...
  - `/api/admin/book-status` and `/api/admin/book-new-flag` — DONE, removed in PR #147;
- обновить docs/env/tests — DONE for books-catalog cleanup.

### PR 7. Remove unused DB session table — DONE in PR #148

- подтверждено, что `session` не используется при JWT strategy;
- `sessions` удалена из schema и test endpoints;
- добавлена и применена миграция `DROP TABLE IF EXISTS "session"`;
- auth smoke/e2e прошли перед merge.

### PR 8. Remove Auth.js account table — DONE in PR #148

- adapter и Google One Tap/helper переключены на `user_identities`;
- проведён account -> user_identities consistency audit;
- `accounts` удалена из runtime schema/code;
- добавлена и применена миграция `DROP TABLE IF EXISTS "account"`;
- auth smoke/e2e для Google OAuth, Google One Tap, magic link, Telegram прошли перед merge.

### PR 9. Remove migration-only books helpers

- DONE before final cleanup: удалены `books.canonical_key`, `books.legacy_sheets_row_id`, `books.source_submission_id`.

### PR 10. Final catalog/admin cleanup — IN PROGRESS 2026-05-25

- удалить `books.size` из schema/API/tests/seed endpoint и production DB;
- восстановить `books.created_at` более близко к историческому значению через первые сигналы из `signup_books`, `book_priorities`, `book_submissions`, `published_at`;
- нормализовать `books.source`: `sheets_import` -> `admin`;
- ужать `books_source_check` до `admin | submission`;
- удалить `books.archived_at`, archive/restore controls и E2E ожидания архива;
- оставить `visibility = published | hidden` единственным состоянием видимости книги;
- удалить owner-specific PostHog opt-out:
  - убрать `POSTHOG_OWNER_EMAILS`;
  - убрать `session.user.isExcludedFromAnalytics`;
  - убрать footer-widget opt-in/opt-out;
  - сохранить `NEXT_PUBLIC_DISABLE_ANALYTICS` и Playwright route-blocking для тестов;
- сохранять активную вкладку админки в query-параметре `tab`, без отдельных страниц;
- обновить unit/e2e тесты под новую модель.

Критерий готовности:

- `npm run lint`, `npm run typecheck`, `npm test` проходят;
- e2e проходят для каталога, book status, admin tab query и UI states;
- production migration применена;
- GitHub CI и Vercel production deploy зелёные;
- в production DB нет `books.size`, `books.archived_at`, `source = 'sheets_import'`.
- удалить `legacy_book_mappings`;
- удалить deprecated one-shot scripts, которые генерировали старую миграционную модель;
- approval flow должен оставаться idempotent через `book_submissions.book_id`.
...
- использовать временный audit/mapping слой на время миграции;
- после финальной проверки удалить migration-only поля и mapping-таблицы;
...
### Риск: преждевременно удалить `account` — закрыт в PR #148

Мера выполнена:

- `IdentityAwareDrizzleAdapter` больше не читает и не пишет `account`;
- перед миграцией добавлены guard checks на соответствие `account` -> `user_identities`;
- auth smoke/e2e прошли перед merge.

### Риск: удалить `session`, но оставить тесты/adapter assumptions — закрыт в PR #148

Мера выполнена:

- подтверждено, что JWT strategy не использует DB sessions;
- `sessionsTable` удалён из активного adapter path;
- test endpoints обновлены.

### Риск: случайно восстановить `book_covers` — закрыт в PR #147

Мера выполнена:

- `cover_url` хранится в `books`;
- внешний кеш обложек не восстановлен;
- `book_covers` удалена из production.
