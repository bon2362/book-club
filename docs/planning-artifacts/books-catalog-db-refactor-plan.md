...
- Этапы 8-9: legacy `session` и `account` убираются отдельным auth cleanup PR после аудита.
- Этап 10: финальная чистка docs/tests/feature docs — DONE for books catalog, auth docs обновляются вместе с auth cleanup.
...
## Этап 8. Удалить legacy `session` table — DONE in auth cleanup branch
...
- миграция `0025_drop_legacy_auth_tables.sql` удаляет таблицу после guard-проверки.
...
- добавлена миграция `DROP TABLE IF EXISTS "session"`;
...
## Этап 9. Удалить legacy `account` table — DONE in auth cleanup branch
...
- миграция `0025_drop_legacy_auth_tables.sql` удаляет таблицу только если все account rows имеют matching identity.
...
- добавлена миграция `DROP TABLE IF EXISTS "account"`;
...
## Этап 10. Финальная чистка тестов и документации — DONE for delivered stages
...
### PR 7. Remove unused DB session table — DONE in auth cleanup branch

- подтверждено, что `session` не используется при JWT strategy;
- `sessions` удалена из schema и test endpoints;
- добавлена миграция `DROP TABLE IF EXISTS "session"`;
- auth smoke/e2e обязателен перед merge.

### PR 8. Remove Auth.js account table — DONE in auth cleanup branch
...
- добавлена миграция `DROP TABLE IF EXISTS "account"`;
- auth smoke/e2e для Google OAuth, Google One Tap, magic link, Telegram обязателен перед merge.
