...
- **Лоадер пересчёта** (`components/nd/MatchingBoardProvider.tsx`): пока сервер пересчитывает сценарии после действия пользователя (добавил/убрал книгу, изменил приоритеты), панели «Сценарии кругов» и «Мои ходы» затемняются и показывают спиннер. Мутирующий компонент зовёт `beginPending()` сразу после своего действия; провайдер снимает loader, когда сервер присылает новый `state_version` (т.е. `router.refresh()` завершился и пришли новые данные) — есть и safety-таймаут на случай зависания. Затемнение+проявление панелей служит и анимацией смены содержимого. Свои действия зовут `router.refresh()` напрямую (не ждут 3с-опрос), поэтому loader покрывает именно время серверного пересчёта.
