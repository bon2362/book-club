...
В чипах других участников статус тоже отображается: «Читаю сейчас» / «Прочитал:а» / «очень хочу» / «хочу» / «без ранга». Ранги 1–3 считаются «очень хочу» и выделяются акцентом; остальные ранжированные записи показываются как «хочу». Tooltip чипа показывает точный rank.
...
3. Строит кандидатные круги фиксированного размера `target_group_size`. Для небольших книг перебирает все комбинации, для больших — ограниченный набор сильных и разнообразных комбинаций, чтобы не потерять слабый, но полезный для покрытия круг.
...
7. Возвращает `scenarioSetOverview`: `scenarios`, `leader`, `totalCount`, `targetGroupSize`. UI показывает каждый сценарий отдельной карточкой с вложенными кругами и участниками «за бортом».
...
Первый экран `/matching` показывает только «Читательские круги» и «Мои ходы», у обеих секций есть внутренний скролл. В «Моих ходах» каждая карточка показывает не только круг, который замкнётся после добавления книги, но и прогноз по сценарию: номер сценария, покрытие участников и список кругов в этом сценарии. При наведении на ход подсвечиваются текущие сценарии, которых касается этот ход: по книге, по прогнозному сценарию или по участникам, которые будут вовлечены.
