...
Карточка хода объясняет **кому поможет действие**. Для каждого хода сервер передаёт `impact`: покрытие до/после, число участников с интересом «очень хочу» до/после и список бенефициаров. Бенефициар — не любой участник нового круга, а только тот, кому стало строго лучше: он был «за бортом» или перешёл на книгу с более сильным интересом (`без ранга → хочу`, `хочу → очень хочу`). В карточке показывается переход `было → стало`, например `за бортом → очень хочу` или `«Заря всего» · хочу → очень хочу`.
