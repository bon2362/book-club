...
Ходы сортируются по силе влияния: прирост покрытия, затем прирост «очень хочу», затем название книги. В UI карточки равнозначны: порядок сохраняется, но первый ход не получает отдельный ярлык «лучший». При hover/focus карточки подсвечиваются только её бенефициары в текущем лидер-сценарии: в чипах кругов или в строке «За бортом». Это точечная связь «мой ход → кому станет лучше», без подсветки всех связанных сценариев.
