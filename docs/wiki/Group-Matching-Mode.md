...
7. Изменения распространяются через **polling**: клиент каждые 3 секунды опрашивает версию сессии (`state_version` в Postgres) и при её росте перезапрашивает состояние. Опрос идёт только пока вкладка активна — в фоновой/свёрнутой вкладке он приостанавливается, а при возврате на вкладку делается немедленный догоняющий запрос (экономит serverless-расход, не теряя свежести). Учитываются не только действия внутри `/matching`, но и изменения участника из каталога, профиля или админки.
...
- **Endpoint версии** (`GET /api/matching/version`): отдаёт `{ version, status, online }` участнику или админу. Клиент `MatchingRealtimeClient` опрашивает его каждые 3 секунды и при росте версии зовёт `router.refresh()`. Никакого payload с типом изменения нет — клиент просто перезапрашивает состояние.
