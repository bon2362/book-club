...
| `optimization_mode` | text | Режим расчёта: `coverage` (по умолчанию) или `satisfaction`; фиксируется при создании сессии |
...
Режим выбирается администратором при создании сессии и дальше не переключается. Это намеренно: участники должны видеть стабильные правила расчёта, а freeze должен фиксировать тот же тип лидера, который был виден на странице `/matching`.
