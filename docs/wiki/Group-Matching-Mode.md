...
- **Имена рядом с псевдонимами:** в основном интерфейсе `/matching` (участники кругов, «за бортом», бенефициары в «Моих ходах») админ видит «Псевдоним (Имя)» — настоящее имя дописывается в скобках. Строится карта `pseudonym → name` из участников и прокидывается только для админа (обычный пользователь видит только псевдонимы). Хелпер `withAdminName` в `components/nd/matching-shared.ts`.
