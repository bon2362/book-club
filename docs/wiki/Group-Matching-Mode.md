...
7. Изменения распространяются через **polling**: клиент опрашивает версию сессии (`state_version` в Postgres) и при её росте перезапрашивает состояние. Опрос «умный» — три механизма экономии serverless-расхода без потери свежести: **(A)** идёт только пока вкладка активна (в фоне приостанавливается, при возврате — немедленный догоняющий запрос); **(B)** частота адаптивна — каждые 3с при ≥2 участниках онлайн, раз в 10с когда участник один (синхронизировать не с кем); **(C)** у зафиксированной (`frozen`) сессии опрос останавливается совсем. Учитываются не только действия внутри `/matching`, но и изменения участника из каталога, профиля или админки.
...
- **Endpoint версии** (`GET /api/matching/version`): отдаёт `{ version, status, online }` участнику или админу. Клиент `MatchingRealtimeClient` опрашивает его и при росте версии зовёт `router.refresh()`. Никакого payload с типом изменения нет — клиент просто перезапрашивает состояние.
- **«Умный polling» — наши ограничения (важно при расследованиях):** клиент намеренно опрашивает не всегда и не с постоянной частотой (`components/nd/MatchingRealtimeClient.tsx` + хук `use-visible-interval.ts` + `lib/matching/poll-interval.ts`):
  - **(A) пауза на скрытой вкладке** — фоновая/свёрнутая вкладка не опрашивает вообще; при возврате — мгновенный догоняющий запрос. Если «realtime не обновляется» — сперва проверь, активна ли вкладка.
  - **(B) адаптивная частота** — 3с при ≥2 онлайн, 10с когда участник один. ⚠️ **idle-интервал жёстко связан с `PRESENCE_WINDOW_MS` (12с)**: опрос версии — это же heartbeat присутствия, поэтому idle обязан быть строго меньше окна, иначе одинокий участник выпадет из «онлайн» для зашедшего. Инвариант закреплён тестом `lib/matching/__tests__/poll-interval.test.ts` — хочешь поднять idle выше 12с, сперва увеличь `PRESENCE_WINDOW_MS`.
  - **(C) стоп на `frozen`** — заморозка терминальна (un-freeze-эндпоинта нет), после финального refresh опрос прекращается.
  - **Почему не SSE/WebSocket:** WebSocket-сервер на Vercel невозможен (serverless), а self-hosted SSE непрерывно жжёт provisioned memory (дороже polling) и in-memory-вариант давал split-brain (см. выше, удалён в #280). Настоящий push требует внешнего realtime-сервиса — задача на будущее в issue **#378**.
- **Онлайн-статус участников (#338)** (`lib/matching/presence.ts`): тот же опрос `/api/matching/version` служит heartbeat'ом — обновляет `matching_session_participants.last_seen_at` звонящего. `online` в ответе = псевдонимы участников, виденных за последние 12 секунд. В попапе «Участники» (`MatchingHeader`) рядом с аватаркой онлайн-участника горит зелёная точка (попап сам опрашивает version каждые 4с, пока открыт); в админ-таблице «Участники» (`AdminMatchingSession`) — аналогичная точка (`online` приходит из `/api/admin/matching/sessions/:id/participants`). `last_seen_at` — телеметрия: `audit_capture` пропускает чисто `last_seen_at`-апдейты (миграция `0042`), как уже делает для `user.last_activity_at` и `user_identities.last_seen_at`, поэтому heartbeat не засоряет audit-лог. Чтение присутствия обёрнуто в try/catch — если колонка ещё не накатана, `version`/`status` работают, presence пуст.
- **Feed** (`lib/matching/realtime/feed.ts`): восстанавливает до 100 значимых событий из `matching_preference_events`; классифицирует только события, которые меняют расклад: `best` и `leftout` («…остался / осталась за бортом» — род глагола согласуется с псевдонимом-животным через `lib/matching/pseudonym-declension.ts`, без гендергепов «остал:ась»). Слово «лучший» в текстах не используется. `best` выпускается при любом значимом изменении лидера (`hasLeaderChanged`), а не только при улучшении — кроме чистой регрессии, которую уже описывает `leftout` (условие в `buildFeedEventsForMutation`: `hasLeaderChanged && (improved || newlyLeftOut.length === 0)`). DTO события `best` несёт `improved: boolean` и `addedCircleBookIds`/`removedCircleBookIds` — по ним фронт (`FeedKeyChip`) разводит подтип чипа: **«Появился круг»** (добавился круг), **«Круг распался»** (исчез), **«Расклад изменился»** (одни круги сменились другими), а при неизменном составе кругов — **«Расклад укрепился»** (↑, `improved`) или **«Расклад ослаб»** (↓, регрессия, warn-палитра). Событие `participant_left` тоже проходит как `best`/`leftout` (его `kind` входит в `isMatchingMutationKind`); псевдоним актора берётся из `metadata.pseudonym`, т.к. строки участника уже нет. Участникам отдаётся публичный DTO: псевдонимы вместо `userId`, а summary сценария содержит только агрегаты (`coveredCount`, `totalCount`, `strongInterestCount`).
...
- **Лоадер пересчёта** (`components/nd/MatchingBoardProvider.tsx`): пока сервер пересчитывает сценарии после действия пользователя (добавил/убрал книгу, изменил приоритеты), панели «Сценарии кругов»/«Мои ходы» затемняются и показывают спиннер, а секции каталога («Остальной каталог»/«Мои книги») приглушаются — пользователь видит, что система «думает». `beginPending()` зовётся **синхронно в момент жеста** (до сетевого запроса), поэтому спиннер появляется сразу по клику, а не после ответа сервера. Пока `pending`, мутирующие обработчики (`if (pending) return`) **не принимают новых действий** — нельзя добавить вторую книгу/перетащить/сменить приоритет, пока не обработается текущее (скролл и чтение сценариев остаются доступны). Провайдер снимает loader, когда сервер присылает новый `state_version` (т.е. `router.refresh()` завершился и пришли новые данные); `endPending()` снимает его сразу, если мутация не удалась, плюс safety-таймаут на случай зависания. Затемнение+проявление панелей служит и анимацией смены содержимого. Свои действия зовут `router.refresh()` напрямую (не ждут 3с-опрос), поэтому loader покрывает именно время серверного пересчёта.
...
После «Войти» пользователь сразу попадает в следующую фазу (gate приоритетов в `satisfaction` или доску) — отдельного промежуточного экрана-подтверждения «Добро пожаловал:а» больше нет. Приветственный экран свёрстан так, чтобы помещаться в один экран без вертикального скролла.
...
Для `satisfaction` действует дополнительный gate: в расчёт попадают только активные signup'ы, у которых есть rank для пары `(userId, bookId)`. Если участник вошёл в active-сессию, но у него нет активных книг или хотя бы одна активная книга ещё без rank, `/matching` показывает промежуточный экран **«Расставь приоритеты»**. На этом экране личный список сохраняется молча без `router.refresh()` после каждого действия; доска открывается только по кнопке **«Войти в сессию»**, когда все активные книги участника имеют приоритет. Перетащить книгу можно за всю область слева от обложки (расширенная зона захвата), а не только за grip-иконку. Admin impersonation (`?as=`) gate обходит и сразу показывает доску.
...
- `best` — лидер-сценарий **значимо изменился** — как в лучшую, так и в худшую сторону. Раньше Лента молчала о регрессиях (показывала только улучшения), из-за чего «добавил книгу» попадал в Ленту, а симметричное «убрал ту же книгу» — нет. Теперь событие создаётся при любом значимом изменении лидера. Флаг `improved` (boolean) в DTO различает направление: чип показывает «Расклад укрепился» (↑, `improved`) или «Расклад ослаб» (↓, регрессия без смены состава кругов), а у регрессии — предупреждающая (warn) палитра. Подтипы по диффу кругов остаются: «Появился круг» / «Круг распался» / «Расклад изменился». Деталь описывает действие актора (с учётом рода: «Барсук добавил…», «Белка убрала…», «… вышел из сессии») и при необходимости суффикс охвата;
- `leftout` — в `leader.leftOut` появился участник, которого там раньше не было. Чтобы не дублировать сигнал, **чистую регрессию, уже описанную выпадением из круга, `best`-событием не дублируем** (`best` выпускается только если расклад улучшился ИЛИ никто новый не выпал).
...
6. Фильтрует Парето-доминируемые сценарии: если другой сценарий сохраняет все те же пары «участник → книга» и добавляет хотя бы одну новую (тот же круг с расширенным составом или дополнительные независимые круги), усечённый вариант скрывается. Это применяется в **обоих** режимах (`coverage` и `satisfaction`), так как сценарий-подмножество хуже по Парето вне зависимости от метрики оптимизации.
7. В режиме `coverage` сортирует сценарии так:
   - `coveredCount DESC`
   - `strongInterestCount DESC`
   - `avgRank ASC`
   - `worstRank ASC`
   - `unrankedCount ASC`
8. В режиме `coverage` по умолчанию возвращает до 10 сценариев. Это ограничение защищает исторический режим от слишком большого набора вариантов в UI и downstream-логике.
9. В режиме `satisfaction` сортирует сначала качество кругов: `avgRank ASC`, `worstRank ASC`, больший размер круга, затем стабильный id. Сценарии сравниваются лексикографически по лучшим кругам; coverage-тиры не применяются.
10. В режиме `satisfaction` дефолтный лимит 10 не применяется: UI получает все сгенерированные ранжированные сценарии, если caller явно не передал `maxResults`.
11. Возвращает `scenarioSetOverview`: `scenarios`, `leader`, `totalCount`, `minGroupSize`, `maxGroupSize`, `mode`. UI показывает каждый сценарий отдельной карточкой с вложенными кругами и участниками без круга.
12. Legacy-обёртки `generateScenarioOverview()` и `generateScenarios()` сохранены для переходного периода и freeze-логики. Они возвращают старую форму `ScenarioCard[]`/`scenarioOverview`, уважают `maxResults` и наследуют `mode`.
13. Tier сценариев: в `coverage` — `leader` = лучший сценарий, `full-coverage` = альтернативы со 100% покрытием, `best-achievable-partial` = лучшие частичные сценарии, `partial` = остальные. В `satisfaction` индекс 0 остаётся `leader` для совместимости, остальные — `partial`, но UI не подаёт leader как окончательный вердикт.
...
- **Имена рядом с псевдонимами:** в основном интерфейсе `/matching` (участники кругов, «за бортом», бенефициары в «Моих ходах», чипы соавторов в секции «Мои книги» и в попапе книги) админ видит «Псевдоним (Имя)» — настоящее имя дописывается в скобках. Строится карта `pseudonym → name` из участников и прокидывается **только для админа** (для не-админа карта `null` — настоящие имена не уходят в клиентские пропсы); хелпер `withAdminName` в `components/nd/matching-shared.ts`. Чипы участников книг рендерят реальный псевдоним и для админа, поэтому карта по псевдониму применима и к ним (`MatchingPersonalList` → `CoSignups`/`MatchingBookDetailModal`).

Если активной сессии нет (например, сессия завершена — `status != 'active'`), обычный пользователь редиректится на каталог `/`; экран со ссылкой на админ-панель видит только админ.
...
Таблица «Аналитика изменений предпочтений»:
- **Деталь для приоритетов** показывает только дифф относительно предыдущего ранжирования (`+Книга (#ранг)`, `−Книга`, `Книга: старый→новый`), а не полный список. Предыдущий снимок берётся из `metadata.previousRankedBookTitles` (пишется в роутах `PATCH /api/matching/priorities` и `PUT /api/priorities`); для исторических событий без снимка и для первичной установки рангов показывается полный нумерованный порядок.
- **Фильтры** по колонкам «Когда», «Событие», «Источник», «Участник», «Актор» (выпадающие списки с уникальными значениями, комбинируются через И).
- **Пагинация** клиентская: по 10 строк, кнопка «Показать ещё» подгружает следующие 10.
...
| `components/nd/MatchingRealtimeClient.tsx` | «Умный» polling-клиент: опрашивает `/api/matching/version` (3с при ≥2 онлайн, 10с в одиночку, пауза на скрытой вкладке, стоп на `frozen`), при росте версии → `router.refresh()` |
| `components/nd/use-visible-interval.ts` | Хук: интервал только при активной вкладке + догоняющий вызов при возврате (слой A) |
| `lib/matching/poll-interval.ts` | Адаптивный интервал по числу онлайн (слой B); инвариант idle < `PRESENCE_WINDOW_MS` |
| `lib/matching/presence-window.ts` | `PRESENCE_WINDOW_MS` (12с) — leaf-модуль без db-зависимостей (нужен и клиенту) |
