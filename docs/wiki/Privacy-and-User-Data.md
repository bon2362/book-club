...
Сайт может запоминать в `localStorage` только последний способ входа, чтобы показать в модалке бейдж «последний способ входа» и раскрыть вторичные способы, если пользователь обычно входит через Google или email. Этот ключ хранит лишь нормализованный provider (`google`, `telegram` или `email`), не содержит email, имени, Telegram username или другого PII и не отправляется на сервер.
...
Админское слияние переносит данные source-профиля в target-профиль только после явного выбора администратора и причины. Переносятся способы входа, записи на книги, приоритеты, заявки, feedback, activity и matching-связи; source user удаляется после переноса. В `user_merge_events` сохраняются source/target snapshots и счётчики переноса. Это операционная история для владельца проекта, не публичные данные.
