...
## Видимость в книжном матчинге

Книжная вкладка доступна только участникам текущей сессии. На карточке видны display names и статусы людей по конкретной книге: простой интерес из глобального шорт-листа, условное согласие, окончательный выбор или назначение. Полный список открывается в книжном popup. Raw `user.id`, контакты и способы входа в public state не передаются; администратор получает отдельные служебные идентификаторы для управления.

Условные и твёрдые намерения, назначения и составы кругов хранятся до удаления matching-сессии. Изменения попадают в смысловой журнал матчинга и глобальный audit log. Закрытие сессии не удаляет данные: оно только переводит страницу в read-only. Удаление аккаунта и административное исключение обрабатываются существующими пользовательскими и matching-процедурами.
...
- В PostHog после входа передаются имя и telegram-username как свойства профиля (ставит сервер),
  но не email — см. `docs/wiki/Analytics-and-PostHog.md` и публичный текст в `content/privacy.md`.
  Запись экрана (session replay) не включена.
...
- PostHog теперь получает не только обезличенный UUID, но и имя с telegram-username (после входа),
  события авторизации (успех/неудача/причина) и подозрение на дубль аккаунта, а также
  JavaScript-ошибки страницы. Email по-прежнему не передаётся, запись экрана не включена.
- В тестовом режиме (`NEXTAUTH_TEST_MODE=true`) серверная отправка событий в PostHog полностью
  отключена, чтобы ночные E2E не заливали боевой проект.
