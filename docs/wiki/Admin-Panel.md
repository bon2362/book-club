...
| Матчинг | Создавать и фиксировать сессии, управлять размером групп, участниками и закреплёнными кругами, смотреть аналитику событий. |

## Матчинг

Администратор видит реальные имена, источник вступления (`self`/`admin`), online и роль участника (`active`/`observer`). Принудительное добавление сохранено для тестирования; интерфейс предупреждает, что оно обходит личный экран disclosure.

Закреплённый круг нельзя изменить по частям. Аварийное действие «Распустить круг» доступно только администратору, требует причины и возвращает в активный расчёт весь состав. В зафиксированной сессии роспуск недоступен.

Секция «Аналитика изменений предпочтений» читает `matching_events`: действия с книгами и рангами, вступление/выход, подтверждения, автоматические переносы, закрепления, роспуск и freeze. В глобальной «Истории изменений» те же операции видны как технические before/after записи audit log. Heartbeat/online там не показывается: это техническая телеметрия, а не изменение решения участника.
