...
| Матчинг | Создавать, закрывать и повторно открывать сессии, управлять участниками и книжными назначениями, смотреть аналитику событий. |

## Матчинг

Администратор видит реальные имена, источник вступления (`self`/`admin`), online и роль участника (`active`/`observer`). Под именем участника выводятся только его текущие договорённости: **«Запись»** для окончательно выбранных книг, **«Авто-запись»** для готовности читать при сборе круга и **«В круге»** для уже назначенных книг. Обычный личный список книг в этой таблице не показывается. Принудительное добавление сохранено для тестирования; интерфейс предупреждает, что оно обходит личный экран disclosure. Книжные назначения и круги редактируются через книжные admin-actions.

Секция «Аналитика изменений предпочтений» читает `matching_events`: действия с книгами и рангами, вступление/выход, намерения, назначения, круги и lifecycle. Исторические сценарные события остаются читаемыми. В глобальной «Истории изменений» операции видны как технические before/after записи audit log. Heartbeat/online не показывается.
