...
- Почта отображается как способ входа, если email identity уже есть; отдельный безопасный flow привязки почты из профиля пока не включён.
...
Каждый неудачный вход через Telegram (неверная HMAC, истёкший `auth_date`, отсутствующий hash) записывается в таблицу `telegram_login_failures`. Журнал хранит причину отказа, разницу времени, Telegram ID/username (если переданы) и IP. Записи не аудируются (actor неизвестен до успешного входа) и автоматически удаляются через 30 дней cron-джобой `telegram-preauth-cleanup`. Журнал полезен для диагностики подозрительных попыток входа и проблем со временем на клиенте.
