...
Любая мутация — через фикстуру в `e2e/fixtures.ts` (`loginAsUser`, `loginAsAdmin`, `createIntroSection`, `createTestBook`, `createMatchingSession`). Фикстура регистрирует cleanup в teardown — данные удаляются даже при падении ассерта. Тесты не редактируют существующие записи; вместо этого создают свою сущность, проверяют, фикстура удаляет.
