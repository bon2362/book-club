...
После записи пользователь может обновить личный статус книги через страницу `/matching`:
...
Хранится в `signup_books.personal_status`. Изменяется через `PATCH /api/signup-books/:bookId/status`. Позволяет движку матчинга не предлагать книгу повторно тем, кто уже её читает или прочитал.
