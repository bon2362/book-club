...
## После исправления статуса книги в закрытом Matching

Миграция `drizzle/0055_release_closed_matching_signup_guard.sql` обновляет DB guard: hard intent и assignment продолжают защищать записи только в текущей `open`/`active` сессии, а `closed`/`frozen` остаются неизменяемой историей и больше не блокируют личный статус книги. Vercel не применяет этот SQL автоматически, поэтому после мержа исправления миграцию нужно один раз выполнить на production Neon через `scripts/apply-migration.ts`. Таблицы и строки миграция не меняет — только заменяет функцию `guard_current_matching_signup_binding()`.
