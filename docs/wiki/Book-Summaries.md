...
На странице опубликованного саммари появляется меню разделов, если в тексте есть минимум два заголовка второго уровня (`##`).

Как это выглядит:

- **На десктопе:** слева от текста — sticky-колонка "Содержание" с названиями разделов. Когда читатель скролит текст, текущий раздел подсвечивается терракотовой линией слева. При клике на название раздела страница плавно скролится к этому месту.
- **На телефоне:** в верхней части текста — закреплённая кнопка с названием текущего раздела и стрелкой. При клике открывается нижний лист со всеми разделами; выбор сворачивает лист и скролит к выбранному месту.

Меню строится автоматически из заголовков в тексте саммари, поэтому авторам не требуется никаких дополнительных действий — стоит только поставить `##` в нужных местах.
...
Внизу каждого опубликованного саммари читатель может поставить необязательную реакцию «Полезно» без регистрации. При нуле видна только подпись, при положительном счётчике — `Полезно · N`. Повторное нажатие снимает реакцию; состояние сохраняется после перезагрузки страницы.

Для вошедшего участника реакция привязана к аккаунту и считается один раз между устройствами. Для гостя сайт использует отдельную first-party HttpOnly cookie только для API саммари. Cookie живёт 12 месяцев с последнего обращения к реакции. В базе хранится не её случайное значение, а SHA-256. После входа реакции текущего браузера переносятся в аккаунт и дубли схлопываются.

Счётчик относится к конкретному тексту автора, а не к книге целиком. Реакция не влияет на рейтинг, сортировку или модерацию; список проголосовавших не показывается. CAPTCHA, fingerprinting, IP-идентификация, уведомления автора и отдельные PostHog-события в v1 отсутствуют.

## Где видны опубликованные саммари

Если у книги есть опубликованные саммари, каталог показывает ссылку вида `/books/dolgoe-nastuplenie/summaries`. На странице книги может быть несколько опубликованных текстов от разных участников. Чтобы тексты разных авторов не сливались в одну простыню, под шапкой книги показывается переключатель авторов: ряд пилюль с аватаром-инициалами и именем, и читателю показывается одно саммари за раз. Выбранный автор отражается в адресе через `?author=…`, поэтому ссылку на конкретного автора можно расшарить, а кнопка «назад» работает. По умолчанию открывается самое свежее саммари. Если саммари всего одно, вместо пилюль показывается строка «Пока одно саммари этой книги». У каждого текста виден короткий мета-блок автора: аватар, имя, оценка времени чтения и дата публикации. Старые ссылки с техническим ID книги продолжают работать и перенаправляются на красивый адрес.
...
- реакции хранятся в `book_summary_helpful_reactions`; удаление саммари или аккаунта каскадно удаляет связанные строки, а гостевой hash маскируется в audit log.

## Ограничения MVP

- Нет комментариев и других типов реакций кроме «Полезно».
