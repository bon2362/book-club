...
Таблицы `book_collections` и `book_collection_items` хранят подборку и её упорядоченный состав. `site_settings` хранит включаемые владельцем настройки; ключ блока подборок — `collections_home_block_enabled`.
