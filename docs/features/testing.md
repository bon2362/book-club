...
После навигации ждите семантический root/состояние конкретного экрана или гидратацию: `await waitForHydration(page)` из `e2e/helpers.ts` (`AppProviders` ставит `data-hydrated` на `<html>` через кадр после монтирования). `waitForLoadState('networkidle')` в спеках не используем: тишина в сети не гарантирует гидратацию, а на polling/realtime страницах и с внешними картинками наступает поздно или никогда. Первый мутабельный click связывайте с ожиданием его HTTP response.

**Проверка «элемента нет» (`toHaveCount(0)`, `not.toBeVisible()`) сразу после `goto`/`reload` проходит мгновенно** — ещё до того, как элемент мог бы появиться. Перед ней нужен положительный якорь, что экран отрисован: соседний элемент, кнопка свёрнутой полки, имя в шапке. После клика, который убирает элемент, якорь не нужен — проверка сама дождётся исчезновения.
...
### Смена сессии при открытой странице приложения (гонка cookie)

`loginAsUser` / `loginAsAdmin` меняют сессию запросом к `/api/test/session`, который
ставит новую cookie. Если в этот момент в браузере открыта страница приложения, её
`SessionProvider` фоново запрашивает `/api/auth/session`, а NextAuth отвечает
...
Симптом коварный: первый запрос сразу после логина отвечает верно, а следующий — уже
от старого пользователя. Внешне это выглядит как «страница `/admin` молча
редиректит на главную» или «в шапке чужое имя». Так падали `book-summaries`
(переключатель авторов) и `circle-calendar` (назначение встречи).

Фикстуры теперь уводят страницу на `about:blank` перед сменой сессии
(`parkPageBeforeSessionSwitch` в `e2e/fixtures.ts`): без открытой страницы приложения
затирать cookie некому. Тесты после логина всё равно делают `goto`/`reload`.

**Если пишешь свой способ смены сессии — уводи страницу с приложения сам**, иначе
получишь тот же плавающий сбой.

### Локаторы кнопок каталога: ограничивай карточками

После добавления книги на странице появляется вторая кнопка «Хочу читать» — селект
статуса чтения (шкала «хочу читать → читаю сейчас → прочитал:а» из #561). Она идёт
раньше карточек в DOM, поэтому `page.getByRole('button', { name: /хочу читать/i }).first()`
кликает по статусу, а не по следующей книге. Так падал `priority-hint`.

Ограничивай локатор карточками: `page.locator('article').getByRole('button', ...)`.

### Ожидания вокруг `POST /api/signup` (профиль и запись на книги)

Сохранение профиля идёт через `/api/signup`, который пишет пользователя, список книг,
аудит и активность. Тестовая БД — удалённая ветка Neon, поэтому каждый запрос внутри
идёт по сети, и ответ локально занимает 4–6 секунд — ровно на границе дефолтных
5 секунд Playwright. Ожидания вокруг этого шага задавай явным таймаутом
(см. `saveProfile` в `e2e/signup.spec.ts`), не полагайся на дефолт.

Дожидаться закрытия формы профиля обязательно: пока она открыта, её оверлей
перехватывает клики по карточкам книг.
