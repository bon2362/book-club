...
- **Приоритеты книг** — таблица `book_priorities` (`userId`, `bookId`, `rank`, `rankSource`, `updatedAt`); обновляется через `POST /api/priorities`; название для UI берётся join-ом из `books.title`; отображается как числа ранга рядом с книгами. Инвариант: у каждой `signup_books` строки с `personal_status IS NULL` всегда есть соответствующая строка `book_priorities` — записанная книга всегда участвует в матчинге, чисел-заглушек `—` больше нет. `rank_source` (`'auto' | 'manual'`) различает автоматически присвоенный ранг (добавление книги, возврат из «Читаю»/«Прочитал:а») от явной ручной пересортировки; подробности — `docs/features/matching.md#обязательные-ранги-rank_source`.
- **Баннер приоритета** — над секцией «Хочу читать» (пока `!prioritiesSet`, `components/nd/ProfileDrawer.tsx`) показывает мягкую формулировку: «Порядок = твой приоритет: книги идут в порядке добавления. Перетащи их так, чтобы сверху были те, что хочется прочитать сильнее — если хочешь уточнить». Явный drag-реордер помечает выставленные ранги `rank_source='manual'` и переключает `users.priorities_set=true`
