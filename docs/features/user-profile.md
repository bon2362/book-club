...
- **Баннер приоритета** — над секцией «Записал:ась» (пока `!prioritiesSet`, `components/nd/ProfileDrawer.tsx`) показывает мягкую формулировку: «Порядок = твой приоритет: книги идут в порядке добавления. Перетащи их так, чтобы сверху были те, что хочется прочитать сильнее — если хочешь уточнить». Явный drag-реордер помечает выставленные ранги `rank_source='manual'` и переключает `users.priorities_set=true`
