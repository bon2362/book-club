...
- До первой модерации редактор находится на техническом адресе `/summaries/{summaryId}/edit`, потому что slug книги ещё не назначен.
- После назначения slug канонический адрес редактора — `/books/{bookSlug}/my-summary/edit`. Старый адрес по UUID остаётся совместимым и перенаправляет на текущий slug обычным серверным redirect, поэтому последующее изменение slug не застревает в браузерном кэше.
...
Администратор может раскрыть строку, поправить `displayName`, `title`, `tldr`, `bodyMarkdown`, указать причину отказа, сохранить изменения, опубликовать или отклонить. При первой модерации он обязан вручную задать уникальный `bookSlug` в формате `lowercase-latin-and-digits`; без slug нельзя ни опубликовать, ни отклонить текст. Slug относится к книге, поэтому общий для саммари разных участников, и позже остаётся редактируемым. Форма показывает неизменяемый UUID саммари, а для ревизии — также её отдельный UUID. Ревизии помечены как `Правки к опубликованному` и показывают текущую публичную версию для сравнения.
...
- `PATCH /api/admin/summaries/{id}` — правка текста/метаданных и `bookSlug` книги.
- `POST /api/admin/summaries/{id}/publish` — публикация.
- `POST /api/admin/summaries/{id}/reject` — отклонение с причиной.
- `PATCH /api/admin/summary-revisions/{id}` — правка ревизии и `bookSlug` книги.
...
- `app/summaries/[id]/edit/page.tsx` — первоначальный редактор и legacy redirect по UUID.
