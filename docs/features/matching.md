...
- `conditional` — «Готов:а читать»: может стоять на нескольких книгах, но не рядом с твёрдым выбором;
...
Карточка (`MatchingBookCard`) показывает статус одной интерактивной строкой из трёх непересекающихся счётчиков-`button`: `hard + assigned`, `conditional` и `interest` (из последней группы исключается `viewerRef`). Пустые группы не рендерятся, каждый счётчик вызывает тот же `onOpenBook(book, currentTarget)`, что и обложка/название. Условная кнопка **«Готов:а читать»** больше не несёт числа и скрывается при `conditionalWouldAssign`. Этот признак вычисляется в canonical read model (`buildPublicBookModeState`), а не эвристикой в React: `shouldFormBook(otherHard, otherConditional + 1)` среди доступных, ещё не назначенных intents (только когда `viewerFree && !viewerHard && !formed`) — т.е. добавление условного согласия зрителя реально пересечёт порог формирования. Метрика **не** использует `intersectionCount`. Сформированная книга помечается кикером `○ круг найден` (или `◆ ваш круг`, если зритель назначен), а единственная левая линия — 3px success-inset у `.is-formed`; отдельного 4px `border-left` у `.is-assigned` нет.
