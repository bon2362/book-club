...
Раздел закрыт по умолчанию: кнопка показывает заголовок «Записаться пока нельзя», число скрытых книг и действие «Показать книги». У кнопки есть `aria-expanded` / `aria-controls`, поэтому её можно раскрыть мышью, Enter или Space; на mobile она занимает всю строку с зоной нажатия не менее 44 px. После раскрытия действие меняется на «Скрыть книги», а шеврон поворачивается. Под кнопкой остаётся подзаголовок «Эти книги остаются в вашем списке, но в подборе не участвуют.» — формулировка одна на оба случая, поэтому не может соврать, в отличие от прежнего «Пока только в вашем списке» + «Когда кто-то ещё выберет эти книги, на них можно будет записаться» (ложь для `reading`-книг, которые вполне могли выбрать и другие).
