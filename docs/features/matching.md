...
Администратор может отправить отдельный сформированный круг читать через `admin_release_circle`. Участники получают `completed_at` и `completed_circle_id` в `matching_session_participants` (миграция `drizzle/0064_partial_matching_completion.sql`), книгу круга — в личный статус `reading`, но назначения, намерения и состав круга не удаляются. Их интересы перестают влиять на счётчики, пороги и разрешённые действия остальных участников (фильтр `isVisibleParticipant` в `buildPublicBookModeState`), но **в админском read model они остаются**: контролы состава (`снять`, выбор круга) адресуют `book.participants`, и без них организатор видел бы человека в круге, не имея способа его тронуть. В админском ответе такой участник помечен `completed: true`, и счётчики карточки его не считают — порог формирования его тоже не учитывает. У самих выпущенных участников доска становится read-only, а книга круга остаётся сверху. `admin_return_circle` снимает завершённость и возвращает только статус `reading` в shortlist с auto-рангом; статус `read` не меняется. Автоматическая пересборка кругов сохраняет выпущенные круги и создаёт новые только для оставшихся назначений.

Счётчик участников в шапке (`MatchingHeader`) считает только тех, у кого `completed_at` пуст: число читают как «сколько людей ещё может попасть в круг», поэтому отправленные читать в него не входят — ни в счётчик, ни в аватары, ни в список в поповере. Публичное состояние отдаёт признак `completed` на каждом участнике, фильтрация — в компоненте.
...
Когда есть открытая сессия (`open`, для совместимости также `active`), вошедший пользователь видит на главной полосу «Идёт матчинг» со ссылкой на `/matching`. Гость и пользователь между сессиями её не видят. Закрытие полосы сохраняется в cookie `matching_strip_dismissed` как id конкретной сессии: оно действует до следующей сессии, но не скрывает новый сезон. Решение о показе делает сервер в `app/page.tsx`, поэтому полоса не появляется после первого кадра; само правило вынесено в чистую `resolveMatchingStripSessionId` (`lib/matching/strip-visibility.ts`) и покрыто тестами, включая возврат полосы в новом сезоне. Признак входа берётся только из `auth()`: отдельно проверять сессионную cookie незачем, а имена cookie библиотеки авторизации в странице каталога — лишняя связь. Ошибка запроса сессии не ломает главную страницу.
...
Редактор расположен в подвкладке «Инструкция» (`/admin?tab=matching&sub=instructions`, `AdminMatchingInstructions`): Markdown-панель, предпросмотр и явная кнопка сохранения. Это глобальная настройка, поэтому она доступна и когда сессия не выбрана. `GET`/`PUT /api/admin/matching/instructions` доступны только администратору; PUT проходит через `withAuditContext`, обновляет `matching_instructions` и вызывает `revalidatePath('/matching')`. Таблица внесена в `AUDITED_TABLES`, а миграция создаёт audit trigger. Проверки: unit/API и focused E2E в `e2e/matching-admin.spec.ts` с reload обеих страниц.

`/admin?tab=matching` собирает `AdminMatchingSession` из небольших компонентов: `AdminMatchingSessionBar` (строка сессии, меню «Сессии (N)» с закрытием по выбору, Esc и клику вне, форма `AdminMatchingNewSessionForm` за пунктом «+ Новая сессия»), `AdminMatchingSummary` (шесть чисел), подвкладки `AdminMatchingInstructions` · `AdminMatchingBookDemand` · `AdminMatchingParticipantsTab` · `AdminMatchingLogTab`. Общие типы и стили — `components/nd/admin-matching-shared.ts`. Подвкладка синхронизирована с `?sub=instructions|demand|people|log` через `router.replace`; неизвестное или пустое значение открывает `demand`. `AdminPanel.selectView` снимает `sub`, когда админ уходит на другую вкладку. Ответы по уже не выбранной сессии отбрасываются по `selectedSessionIdRef`, чтобы переключение не смешивало данные.
...
**Участники** — таблица «Имя · Роль · Источник · Вступил · действие». Списки книг под именем убраны из основного вида: раскрытие включается кликом по строке целиком (`role="button"`, `aria-expanded`, Enter/Space; шеврон `▸/▾` слева от имени, у участника без книг строка инертна), а блок с именем и кнопка «убрать» гасят всплытие — клик по имени по-прежнему ведёт в `/matching?as=<userId>`. Внутри — шесть групп `participantBookGroups()`: `want` (ранжированный wishlist), `unranked` (wishlist без ранга), `hard`, `conditional`, `assigned` («В круге» / «Читает» по `completedAt`) и `reading`; пустые группы не рендерятся, каждая книга — отдельная строка с рангом `#N` или `·`, без кавычек. Данные те же: `GET …/participants` отдаёт `wishlist` (весь ранжированный «Хочу читать», включая книги без пересечений) и `readingNow`. Ручное добавление свёрнуто вместе с предупреждением о раскрытии имени, «убрать» видно на hover и `focus-within` и спрашивает `window.confirm`. Мутации — только в открытой сессии.
...
Полка объединяет две причины, по которым книга временно недоступна для записи, — оба случая рендерятся группой `data-testid="matching-tail-group"` (issue #560; до переименования — `matching-viewer-only-divider`, обозначавший только первый случай):

1. **`waiting`** — книгу выбрали меньше трёх участников, включая текущего. До порога не хватает интереса, авто-записи, окончательной записи или назначения других участников;
...
Полка закрыта по умолчанию. Кнопка `matching-tail-toggle` показывает до трёх обложек скрытых книг, заголовок «На эти книги пока записаться нельзя» и подзаголовок «Ждём, пока их добавят другие участники». Счётчика книг нет. У кнопки есть `aria-expanded` / `aria-controls`, поэтому её можно раскрыть мышью, Enter или Space; на mobile она занимает всю строку с зоной нажатия не менее 44 px. При раскрытии поворачивается шеврон, появляются карточки и пояснение: «Книги остаются в вашем списке. Как только они появятся минимум у трёх участников, здесь можно будет записаться.»
...
- `waiting` → «ЖДЁМ ДРУГИХ» + «Когда книга будет в списках минимум у трёх участников — можно будет записаться»;
...
На обеих карточках хвоста нет кнопки записи и меню авто-записи (`allowedActions` для `reading`-книги — все `false`); подробности книги остаются доступны. Для `waiting`-книг кнопка записи появляется, когда у книги есть минимум два других участника с интересом, авто-записью, окончательной записью или назначением. Если интерес затем опускается ниже порога, ранее сделанная запись сохраняется вместе с доступной отменой, а для авто-записи — отдельной кнопкой «Отменить авто-запись». Правила формирования кругов и серверные действия не меняются.
