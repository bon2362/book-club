...
`MatchingHeader` возвращает навигацию в каталог, название и состояние сессии, размеры групп, дедлайн, роль viewer, список участников с heartbeat-индикаторами, выход и административное редактирование размеров. `MatchingWorkspace` держит сценарии во внутренней прокрутке, а книжную вкладку выводит в естественном document flow без внутреннего height-cap; pending-state пересчёта не создаёт пустого `90svh`-блока. Feed, mode selector, pseudonym и «Мои ходы» намеренно отсутствуют.
...
Книжная вкладка использует компактные `.nd-mb-card`: обложка имеет фиксированную геометрию 54×76 и обязательно служит positioning context для `next/image` с `fill`. Без `position: relative` изображение привязывается к карточке и перекрывает доску; этот инвариант проверяется через computed style и `boundingBox()` в `e2e/matching-layout.spec.ts`. Строка агрегатов показывает три непересекающихся счётчика (записались / готовы читать / ещё у N в списке), а мягкая авто-запись задаётся чекбоксом в меню сплит-кнопки «Записаться ▾». Сформированная книга получает лёгкую success-заливку из токена. Аватарная воронка на карточке не рендерится, поскольку повторяла те же агрегаты; подробные участники и их компактные статусы остаются в `MatchingBookDetailModal`, где имя по hover/focus/tap раскрывает tooltip с местом книги в списке участника. В `adminMode` пользовательский `.nd-mb-actions` отсутствует, и после мета-строки сразу идёт `MatchingBookAdminControls`.
