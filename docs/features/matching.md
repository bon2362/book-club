...
## Книги без пересечений

Книги, которые пока выбрал только текущий участник, остаются видимыми в разделе «Пока только в вашем списке». Под заголовком поясняется: «Когда кто-то ещё выберет эти книги, на них можно будет записаться». На таких карточках нет новой записи и меню авто-записи; подробности книги доступны.

Кнопка записи появляется, когда у книги есть хотя бы один другой участник с интересом, авто-записью, окончательной записью или назначением. Если последнее пересечение исчезло, ранее сделанная запись сохраняется вместе с доступной отменой. Для прежней авто-записи остаются статус и отдельная кнопка «Отменить авто-запись». Правила формирования кругов и серверные действия не меняются.
