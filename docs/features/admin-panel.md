...
Карточка пользователя в админке показывает внутренний ID пользователя: по нему можно кликнуть, чтобы скопировать ID в буфер обмена. Для слияния администратор сначала копирует ID основного аккаунта, затем открывает карточку дубля, вставляет этот ID в поле «ID аккаунта, который оставить», при необходимости указывает причину и подтверждает действие кнопкой `Merge to user`. API `POST /api/admin/users/merge` требует `sourceUserId` и `targetUserId`, принимает опциональный `reason`, запрещает self-merge и не позволяет админу слить собственный аккаунт как source.
