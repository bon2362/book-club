...
- **Вкладка «Участники»** — показывает пользователей и их записи из Postgres (`user` + `signup_books`); администратор может удалить пользователя через `DELETE /api/admin/delete-user` или слить дубль в основной аккаунт через `POST /api/admin/users/merge`
...
Карточка пользователя в админке содержит действие «Слить дубль». Администратор выбирает целевого пользователя, указывает причину и подтверждает действие. API `POST /api/admin/users/merge` требует `sourceUserId`, `targetUserId` и `reason`, запрещает self-merge и не позволяет админу слить собственный аккаунт как source.
