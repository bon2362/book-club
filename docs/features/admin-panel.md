...
- **Вкладка «Участники»** — показывает пользователей и их записи из Postgres (`user` + `signup_books`); поиск работает по имени и Telegram. Если у пользователя есть хотя бы одна книга с личным статусом `reading`, строка получает мягкий фон, а бейдж «Читает» появляется при наведении. Администратор может удалить пользователя через `DELETE /api/admin/delete-user` или слить дубль в основной аккаунт через `POST /api/admin/users/merge`
...
В карточке участника администратор меняет личный статус через канонический `PATCH /api/signup-books/:bookId/status?as=:userId`. После успеха карточка перечитывается через `GET /api/admin/users/:userId`, поэтому секция и ранг показывают серверное состояние. Ответы `4xx/5xx` выводятся внутри карточки понятным сообщением; локально статус при отказе не подменяется. Совместимый `PATCH /api/admin/signup-books` поддерживает тот же инвариант рангов.
