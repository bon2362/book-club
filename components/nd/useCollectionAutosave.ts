...
export function useCollectionAutosave({ initialId, content, enabled, delayMs = 800, onCreated }: { initialId: string | null; content: CollectionSnapshot; enabled: boolean; delayMs?: number; onCreated?: (id: string) => void }) {
  ...
  const latest = useRef(JSON.stringify(content)); const saved = useRef(JSON.stringify(content)); const idRef = useRef(initialId); const run = useRef<Promise<void> | null>(null); const again = useRef(false)
  latest.current = JSON.stringify(content)
  const save = useCallback(async () => {
    if (run.current) { again.current = true; return run.current }
    ...
    })().finally(() => { run.current=null }); await run.current
    if (again.current) { again.current = false; await save() }
  }, [onCreated])
