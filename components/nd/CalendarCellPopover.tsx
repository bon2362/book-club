...
export default function CalendarCellPopover({
  ...
  return (
    <div role="dialog" aria-modal="true" style={{ position: 'fixed', right: 24, bottom: 24, zIndex: 40, width: 'min(320px, calc(100vw - 32px))', background: 'var(--bg-input)', border: '1px solid var(--hair)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--shadow-pop)', padding: 14 }}>
      ...
      <h4 style={{ margin: '0 0 2px', fontFamily: 'var(--nd-serif)', fontWeight: 400, fontSize: '1.02rem' }}>{formatDate(date, timeZone)}</h4>
      <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', marginBottom: 10 }}>{formatTime(date, timeZone)} · свободны <b style={{ color: canSchedule ? 'var(--success)' : 'var(--accent)' }}>{cell.freeRefs.length} из {Math.max(1, markedCount)}</b></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, borderTop: '1px solid var(--hair)', paddingTop: 10 }}>
        {canEdit && canSchedule && <button type="button" onClick={onSchedule} style={primaryButtonStyle}>Назначить встречу на {formatTime(date, timeZone)}</button>}
