        ...
        <div className="nd-scenario-circles">
          {scenario.circles.map(circle => {
            ...
            return <article key={circle.circleKey} data-testid="matching-circle" className="nd-scenario-circle" style={{ padding: '0.75rem', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              ...
              <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>{circle.members.map(member => <li key={member.ref} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', minWidth: 0 }}><ParticipantInterestChip userId={member.ref} displayName={member.displayName} rank={member.rank} />{member.confirmed && <span aria-label={`${member.displayName}: подтвердил`} title={`${member.displayName}: подтвердил`} style={{ color: 'var(--success)', fontSize: '0.82rem', lineHeight: 1, flexShrink: 0 }}>✓</span>}</li>)}</ul>
