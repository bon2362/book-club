...
export default function MatchingLockedCircles({ circles, viewerLockedCircleKey, booksById = {} }: Props) {
  ...
  const card = (circle: LockedCircle) => { const book = booksById[circle.bookId]; return <article key={circle.circleKey} data-testid="matching-locked-circle" style={{ borderTop: '3px solid var(--success)', borderRight: '1px solid var(--hair)', borderBottom: '1px solid var(--hair)', borderLeft: '1px solid var(--hair)', padding: '0.8rem', display: 'grid', gridTemplateColumns: '54px minmax(0, 1fr)', gap: '0.7rem' }}><button type="button" className="nd-book-link" aria-label={`Открыть книгу «${book?.title ?? 'Книга круга'}»`} onClick={() => show(circle)}><CoverImage coverUrl={book?.coverUrl ?? null} title={book?.title ?? 'Книга круга'} author={book?.author ?? ''} /></button><div><button type="button" className="nd-book-title-button" onClick={() => show(circle)}>{book?.title ?? 'Книга круга'}</button><div style={{ fontSize: '0.74rem', color: 'var(--text-muted)', marginTop: '0.15rem' }}>{book?.author ?? ''}</div><ul style={{ margin: '0.5rem 0 0', padding: 0, listStyle: 'none', display: 'flex', flexWrap: 'wrap', gap: '0.35rem 0.8rem' }}>{circle.members.map(member => <li key={member.ref} style={{ fontSize: '0.8rem', color: 'var(--text-body)' }}>{member.displayName} <span aria-label="подтвердил" style={{ color: 'var(--success)' }}>✓</span></li>)}</ul></div></article> }
  return <section data-testid="matching-locked-circles" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
    {own && <div data-testid="matching-own-locked-circle" style={{ borderLeft: '3px solid var(--success)', paddingLeft: '1rem' }}><h2 style={{ margin: '0 0 0.3rem', fontFamily: 'var(--nd-serif)', fontSize: '1.25rem' }}>Ваш круг</h2><p style={{ margin: '0 0 0.8rem', color: 'var(--text-muted)', fontSize: '0.8rem' }}>Все участники подтвердили состав. Вы больше не участвуете в расчётах, но можете наблюдать за сессией.</p>{card(own)}<span style={{ display: 'inline-block', marginTop: '0.5rem', color: 'var(--success)', fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Вы наблюдаете</span></div>}
