...
- `назначен` — администратор закрепил книгу за участником, но ещё не разместил его в круге;
...
В `assignedCount` и сводку «в кругах» входят только участники с конкретным кругом. Приоритет отображения статуса: `в круге` → `назначен` → `записался` → `авто-запись` → `в списке`.
