...
**Ask First:** Любое изменение, которое начинает рекомендовать конкретные действия участникам, автоматически понижает/повышает людей из-за статуса `reading`, меняет правила формирования книги, меняет текущую таблицу участников вместо добавления отдельного блока или добавляет мутации в новый coordination endpoint.
