...
/**
 * Записывает полный список книг пользователя. Общая точка для `POST /api/signup`
 * и записи одной книги со страницы подборки: учитывает активную сессию матчинга
 * (`replace_signup`) и инвариант рангов.
 */
export async function saveSignupSelection(session: AuthSession, input: SignupSelectionInput): Promise<NextResponse> {
  ...
  try {
    if (activeSessionId) {
      result = await previewSignupByBookIds(pgUserId, selectedBookIds)
      ...
      result = await withAuditContext(auditCtx, async (tx) => {
        const upsert = await upsertSignupByBookIds(pgUserId, selectedBookIds, tx)
  ...
  await bestEffortRecordUserActivity(pgUserId, 'profile_submitted', {
    source: 'api',
    sourceId: pgUserId,
    dedupeKey: buildUserActivityDedupeKey([
      'api',
      'profile_submitted',
      pgUserId,
      name.trim(),
  ...
  // Уведомление для дайджеста (в тест-режиме не шлём — тесты работают с настоящей базой).
  if (result.addedBooks.length > 0 && process.env.NEXTAUTH_TEST_MODE !== 'true') {
    const addedBooks = result.addedBooks
    ...
    withAuditContext(auditCtx, async (tx) =>
      tx.insert(notificationQueue).values({
        ...
        contacts: contacts.trim(),
        addedBooks: JSON.stringify(addedBooks),
        isNew,
      }),
    ).catch(() => {
