        ...
        <Section title="Статистика кода">
          ...
          >
            <p style={{ ...bodyStyle, fontStyle: 'italic', margin: 0, color: 'var(--text-secondary)' }}>
              ~60% кода — UI-компоненты. Тесты выросли до 14% кодовой базы:
              unit-тесты покрывают API-маршруты и бизнес-логику, E2E (Playwright)
              проверяют ключевые сценарии — авторизацию и запись на книгу.
              Всё запускается в CI при каждом пуше.
            </p>
          </div>
        </Section>
...
const FEATURES = [
  ...
  { title: 'Автотесты и CI/CD', desc: '45 unit-тестов для критичных API-маршрутов и бизнес-логики, E2E-тесты Playwright для флоу авторизации и записи. Все тесты запускаются в GitHub Actions при каждом пуше' },
]
...
const DECISIONS = [
  ...
  },
  {
    ...
    body: 'E2E-тесты не обращаются к Google Sheets и реальной БД — флаг NEXTAUTH_TEST_MODE=true включает моки на уровне route handlers. Это позволяет гонять полный сценарий записи на книгу в CI без реальных учётных данных.',
  },
  {
...
const STATS = [
  { value: '~4 200', label: 'строк кода' },
  { value: '18', label: 'реализованных функций' },
  { value: '8', label: 'сервисов и технологий' },
  { value: '45+', label: 'автотестов' },
]

const CODE_ROWS = [
  { category: 'UI-компоненты (страницы, карточки, формы, хедер)', lines: 2500 },
  { category: 'Бизнес-логика (Sheets, авторизация, БД, поиск)', lines: 500 },
  { category: 'API-маршруты и серверные страницы', lines: 430 },
  { category: 'Тесты (unit + E2E)', lines: 600 },
  { category: 'Стили и конфигурация', lines: 200 },
]
