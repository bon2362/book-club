...
/** Чужую подборку не показываем даже фактом существования — отвечаем как на несуществующую. */
async function loadOwnCollection(id: string, session: AuthSession) {
  const record = await loadCollectionById(id)
  return record && isCollectionOwner(record, viewerFromSession(session)) ? record : null
}
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    const record = await loadOwnCollection(params.id, session as AuthSession)
    ...
    const saved = await withAuditContext(
      ...
      (tx) => saveCollectionContent(tx as never, { id: params.id, content, by: 'author', now: new Date() }),
    )
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    await withAuditContext(
      collectionAuditContext(session as AuthSession, 'collections'),
      (tx) => deleteCollection(tx as never, params.id),
    )
