...
import { eventMembershipPatchSchema, eventMembershipSchema } from '@/lib/timeline/contents'
    ...
    async (tx) =>
      tx
        ...
        .onConflictDoUpdate({
          ...
          set: { note: parsed.data.note, visible: parsed.data.visible, updatedAt: new Date() },
        }),
  )
...
/** Меняет только локальную видимость связи, не трогая общее событие. */
export async function PATCH(req: NextRequest, { params }: Context) {
  ...
  const parsed = eventMembershipPatchSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json({ error: firstZodMessage(parsed.error) }, { status: 400 })
  }
  ...
  await withAuditContext(
    {
      ...
      actorLabel: session.user.name ?? session.user.contactEmail ?? null,
      source: 'admin',
      reason: parsed.data.visible
        ? 'Показ события на ленте времени'
        : 'Скрытие события на ленте времени',
    },
    async (tx) =>
      tx
        .update(timelineEvents)
        .set({ visible: parsed.data.visible, updatedAt: new Date() })
        .where(
          and(
  ...
  return NextResponse.json({
    success: true,
    data: { timelineId: params.id, eventId: params.eventId, visible: parsed.data.visible },
  })
}
