...
describe('POST /api/signup', () => {
  ...
  it('возвращает 400 при пустом name', async () => {
    jest.spyOn(authModule, 'auth').mockResolvedValue({ user: { email: 'test@test.com' } } as any)
  ...
  it('возвращает 200 при успешной записи', async () => {
    jest.spyOn(authModule, 'auth').mockResolvedValue({ user: { email: 'test@test.com' } } as any)
    jest.spyOn(signups, 'upsertSignup').mockResolvedValue({ isNew: true, addedBooks: ['Book A'] })
  ...
  it('обрезает пробелы в name и contacts', async () => {
    jest.spyOn(authModule, 'auth').mockResolvedValue({ user: { email: 'test@test.com' } } as any)
    jest.spyOn(signups, 'upsertSignup').mockResolvedValue({ isNew: false, addedBooks: [] })
